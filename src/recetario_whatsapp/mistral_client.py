...
class MistralClient:
            ...
            try:
                ...
                try:
                    ...
                    if isinstance(resultado, dict) and "recetas" in resultado:
                        ...
                        return {"recetas": [resultado] if resultado.get("es_receta") else []}
                    ...
                    recetas_encontradas = self._extraer_recetas_simple(texto_bloque, respuesta)
                ...
                error_str = str(e)
                es_capacidad = any(term in error_str.lower() for term in ["429", "capacity", "service tier"])
                if es_capacidad and intento < self.max_reintentos - 1:
                    espera = self.reintento_delay * (intento + 1)
                    print(f"  ⏳ Error de capacidad, reintento {intento + 1}/{self.max_reintentos - 1} en {espera:.1f}s")
                    time.sleep(espera)
                    continue
                ...
                if es_capacidad:
                    ...
                    if recetas_fallback:
                        return {"recetas": recetas_fallback, "warning": "fallback_regex"}

                return {"recetas": [], "error": f"Error en la API de Mistral: {error_str}"}

        return {"recetas": [], "error": "Error en la API de Mistral tras múltiples reintentos"}
    ...
    def _extraer_recetas_simple(self, texto_original: str, respuesta_mistral: str) -> List[Dict[str, Any]]:
        """
        ...
        for match in matches:
            ...
            tiene_ingredientes = bool(re.search(r'\d+\s*[gkgmltazas]?\s+[a-zA-Z]', contenido)) or \
                               bool(re.search(r'\d+\s+[a-zA-Z]', contenido))

            # Buscar pasos de preparación
            tiene_pasos = bool(re.search(r'\d+\.|\-|\•|hornear|cocinar|freír|mezclar', contenido_lower))
            ...
            if tiene_ingredientes or tiene_pasos:
                ...
                for i, linea in enumerate(lineas):
                    if re.search(r'pasos?|preparación|instrucciones|modo de hacer', linea.lower()):
                        ingredientes_end = i
                        break
                ...
                try:
                    ...
                    fecha_obj = datetime.strptime(f"20{fecha.replace('/', '-')}", '%Y-%d-%m')
                ...
                receta = {
                    ...
                    "tiene_foto": "imagen" in contenido_lower or "foto" in contenido_lower,
